...
public class SolutionWriter {
    ...
    private static void writeTasksForServer(RBIntervalNode node, BufferedWriter writer) throws IOException {
        if (node == null) {
            return;
        }
        // Inorder traversal
    ...
    public static void writeSolutionToFile(RBSolution solution, String testInstanceName, String heuristicName, int totalTime) {
        try {
            String directoryPath = "RBSolutionsBestCapacity"; // Change this to "RBSolutions" to write all solutions for BCHT Heuristic
            File directory = new File(directoryPath);
            if (!directory.exists()) {
                directory.mkdir();
            }
            ...
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
                List<RBIntervalTree> intervalTrees = solution.getIntervalTrees();
